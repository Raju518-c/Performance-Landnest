...
class GenerateDummyPropertiesAPI(APIView):
    ...
    def post(self, request):
        try:
            fake = Faker('en_IN')
            total = int(request.data.get('count', 500000))
            batch_size = 1000
            ...
            with transaction.atomic():
                for batch_start in range(0, total, batch_size):
                    ...
                    for i in range(batch_start, batch_end):
                        # Random user_id (15000-800000) and category_id (1-10)
                        user_id_id = random.randint(15000, 800000)
                        ...
                        property_dt = Property(
                            ...
                            lat = str(round(random.uniform(8.0, 37.0), 6)),
                            long = str(round(random.uniform(68.0, 97.0), 6)),
...
@method_decorator(csrf_exempt, name='dispatch')
class GenerateFakeVendorsAPIView(APIView):

    def post(self, request):
        from faker import Faker
        import random
        from users.models import Vendors, VendorWorkImage

        fake = Faker('en_IN')

        # Default quantities
        vendors_count = int(request.data.get('vendors_count', 250000))
        work_images_count = int(request.data.get('work_images_count', 350000))

        # Choices for Vendors
        professions = [
            'Architect', 'Interior Designer', 'Civil Engineer', 'Construction Contractor',
            'Real Estate Agent', 'Property Lawyer', 'Home Inspector', 'Land Surveyor',
            'Electrician', 'Plumber', 'Carpenter', 'Painter', 'Landscaper',
            'HVAC Technician', 'Roofing Contractor', 'Flooring Specialist',
            'Structural Engineer', 'Project Manager', 'Building Inspector',
            'Real Estate Appraiser', 'Mortgage Broker', 'Property Manager'
        ]

        batch_size = 5000
        created_vendors = 0
        created_work_images = 0

        # Step 1: Create Vendors
        print("Creating Vendors...")
        while created_vendors < vendors_count:

            bulk_data = []
            current_batch = min(batch_size, vendors_count - created_vendors)

            for i in range(current_batch):
                # Use user_id range 2001-800000
                user_id_value = random.randint(2001, 800000)
                
                vendor_obj = Vendors(
                    user_id_id=user_id_value,
                    name=fake.name(),
                    ...
                    profile='media/vendor_profiles/default_profile.jpg',
                    description=fake.text(max_nb_chars=500)
                )

                bulk_data.append(vendor_obj)

            # Save each Vendor individually
            for vendor_obj in bulk_data:
                vendor_obj.save()

            created_vendors += current_batch
            print(f"Created Vendors: {created_vendors}")

        # Step 2: Create VendorWorkImages
        print("Creating VendorWorkImages...")
        all_vendors = list(Vendors.objects.all())
        
        while created_work_images < work_images_count:
            current_batch = min(batch_size, work_images_count - created_work_images)
            
            for i in range(current_batch):
                # Pick a random Vendor
                vendor = random.choice(all_vendors)
                
                VendorWorkImage.objects.create(
                    vendor=vendor,
                    image='media/vendor_work_images/sample_work.jpg'
                )
            
            created_work_images += current_batch
            print(f"Created VendorWorkImages: {created_work_images}")

        return Response(
            {
                "message": "Fake vendor records created successfully",
                "vendors": created_vendors,
                "vendor_work_images": created_work_images
            },
            status=status.HTTP_201_CREATED
        )
